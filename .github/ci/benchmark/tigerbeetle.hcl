...
./zig/zig build install -Dcpu=x86_64_v3+aes -Doptimize=ReleaseSafe
...
exec ./tigerbeetle start --cache-grid=1000GB --addresses=${var.addresses} /tank/{{ env "NOMAD_JOB_ID" }}.tigerbeetle
    EOF
